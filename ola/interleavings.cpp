...
string a,b;
int la,lb;
void printInterleaving(int idx1, int idx2, string prefix){

    if(prefix.length() == la + lb){
        cout<<prefix<<endl;
        return;
    }
    if(idx1 < la){
        string str1 = prefix + a[idx1];
        printInterleaving(idx1+1,idx2, str1);
    }
    if(idx2 < lb){
        string str2 = prefix + b[idx2];
        printInterleaving(idx1,idx2+1, str2);
    }
}
